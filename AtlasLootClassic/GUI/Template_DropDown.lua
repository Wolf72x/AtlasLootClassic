...
do
	...
	local function CreateButton(dropdown, cat, buttonInfo, catIndex)
		...
		if not frame then
			...
			frame.check:SetTexture("Interface\\AddOns\\AtlasLootClassic\\Images\\ui-dropdownradiochecks")
	...
	local function CreateCategory(dropdown, catTab, parent)
		...
		if not frame then
			...
			frame = CreateFrame("Frame", frameName, nil)
...
function GUI.CreateDropDown()
	...
	self.frame = CreateFrame("Button", frameName, nil)
