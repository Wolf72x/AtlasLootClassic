...
local LOOT_BORDER_BY_QUALITY_AL = {
    [0] = ALPrivate.IMAGE_PATH.."loottoast-itemborder-gray",   -- Poor
    [1] = ALPrivate.IMAGE_PATH.."loottoast-itemborder-white",  -- Common
    [2] = ALPrivate.IMAGE_PATH.."loottoast-itemborder-green",  -- Uncommon
    [3] = ALPrivate.IMAGE_PATH.."loottoast-itemborder-blue",   -- Rare
    [4] = ALPrivate.IMAGE_PATH.."loottoast-itemborder-purple", -- Epic
    [5] = ALPrivate.IMAGE_PATH.."loottoast-itemborder-orange", -- Legendary
	[6] = ALPrivate.IMAGE_PATH.."loottoast-itemborder-artifact", -- Artifact
	[7] = ALPrivate.IMAGE_PATH.."loottoast-itemborder-heirloom", -- Heirloom
}
...
function Button.Init()
    ...
    elseif locale == "zhTW" then WOW_HEAD_LOCALE = "cn"
    end
end
...
local function Button_Overlay_SetQualityBorder(self, qualityID)
	...
	self:SetTexture(LOOT_BORDER_BY_QUALITY_AL[qualityID] or LOOT_BORDER_BY_QUALITY_AL[0])
...
local function Button_Overlay_SetAchievementBorder(self, set, isGuildAc)
	if not set then
		...
		self:SetPoint("TOPLEFT", self.icon, "TOPLEFT", -1, 1)
		self:SetPoint("BOTTOMRIGHT", self.icon, "BOTTOMRIGHT", 1, -1)
		return
	end


	self:SetTexture("Interface\\ACHIEVEMENTFRAME\\UI-Achievement-IconFrame")
	self:SetTexCoord(0, 0.5625, 0, 0.5625)
	

	

	self:SetPoint("TOPLEFT", self.icon, "TOPLEFT", -3, 3)
	self:SetPoint("BOTTOMRIGHT", self.icon, "BOTTOMRIGHT", 3, -3)
...
function Button:Create()
	...
	button.completed:SetDrawLayer("ARTWORK")
	...
	button.favourite:SetDrawLayer("OVERLAY")
	button.favourite:SetPoint("TOPRIGHT", button.icon, 4, 4)
	button.favourite:SetHeight(16)
	button.favourite:SetWidth(16)
	...
	button.secButton.icon:SetDrawLayer("BACKGROUND")
	...
	button.secButton.overlay:SetDrawLayer("OVERLAY")
	button.secButton.overlay:SetPoint("TOPLEFT", button.secButton.icon, "TOPLEFT",  -1, 1)
	button.secButton.overlay:SetPoint("BOTTOMRIGHT", button.secButton.icon, "BOTTOMRIGHT",  1, -1)
	...
	button.secButton.completed:SetDrawLayer("OVERLAY")
	...
	button.secButton.count:SetDrawLayer("OVERLAY")
	...
	button.secButton.pvp:SetDrawLayer("OVERLAY")
	...
	button.secButton.favourite:SetDrawLayer("OVERLAY")
	button.secButton.favourite:SetPoint("TOPRIGHT", button.secButton.icon, 4, 4)
	button.secButton.favourite:SetHeight(16)
	button.secButton.favourite:SetWidth(16)
...
function Button:CreateSecOnly(frame)
	...
	-- secButtonTexture <texture>
	button.secButton.icon = button.secButton:CreateTexture(buttonName.."_secButtonIcon", "BACKGROUND")
	button.secButton.icon:SetDrawLayer("BACKGROUND")
	button.secButton.icon:SetAllPoints(button.secButton)
	button.secButton.icon:SetTexture("Interface\\Icons\\INV_Misc_QuestionMark")
	...
	-- secButtonOverlay <texture>
	button.secButton.overlay = button.secButton:CreateTexture(buttonName.."_secButtonOverlay")
	button.secButton.overlay:SetDrawLayer("BORDER")
	...
	button.secButton.count = button.secButton:CreateFontString(buttonName.."_secCount", "ARTWORK", "AtlasLoot_ItemAmountFont")
	button.secButton.count:SetDrawLayer("ARTWORK")
	...
	button.secButton.pvp = button.secButton:CreateTexture(buttonName.."_secButtonPvp")
	button.secButton.pvp:SetDrawLayer("ARTWORK")
	button.secButton.pvp:SetPoint("BOTTOMRIGHT", button.secButton.icon, "BOTTOMRIGHT", -3, 3)
	button.secButton.pvp:SetHeight(13)
	button.secButton.pvp:SetWidth(13)
	button.secButton.pvp:Hide()
