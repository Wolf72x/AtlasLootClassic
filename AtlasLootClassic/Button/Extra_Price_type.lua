...
local function SetContentInfo(frame, typ, value, delimiter)
	...
	if PRICE_INFO[typ] then
		...
		elseif PRICE_INFO[typ].itemID then
			...
			PRICE_INFO[typ].icon = select(10, GetItemInfo(PRICE_INFO[typ].itemID))
			if(PRICE_INFO[typ].icon) then
...
function Price.OnSet(mainButton, descFrame)
	if FIRST_RUN then
		for k,v in pairs(PRICE_INFO) do
			...
			if v.itemID and not v.icon then
				local itemName, _, _, _, _, _, _, _, _, icon = GetItemInfo(v.itemID) 
