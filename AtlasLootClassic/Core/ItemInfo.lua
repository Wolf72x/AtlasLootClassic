...
local function Init()
	...
	for iC = 0, #LOC_DATA do
		...
		local className = class.__name

		--NewLocData.en[class.__name] = className
		NewLocData.loc[className] = className

		for isC = 0, #class do
			local subClass = class[isC]
			local name = ITEM_SUBCLASSES[iC] and ITEM_SUBCLASSES[iC][isC]
			if name and not NewLocData.loc[subClass] then
				NewLocData.loc[className .. name] = name
			end
		end
	end
