...
local GetServerTime = _G.GetTime
...
local function PopulateListBiS(db, dest)
    ...
    for i = 1, GetNumEquipmentSets() do
        local name, _, _, _, _, _, setID = GetEquipmentSetInfo(i)  -- old API
        local itemSetItems = { GetEquipmentSetItemIDs(i) }           -- returns all 19 slots (or nil if empty)
...
local function ChatFilterFunc(_, event, msg, player, l, cs, t, flag, channelId, ...)
	...
	repeat
		...
		if (characterName and displayName) then
			...
			newMsg = newMsg.."|Hspell:1|h|cFF8800FF["..characterName.." |r|cFF8800FF- "..displayName.."]|h|r";
...
function Favourites.OnInitialize()
	...
	hooksecurefunc("SetItemRef", function(link, text)
		if (link == "spell:1") then
			local _, _, characterName, displayName = text:find("|Hspell:1|h|cFF8800FF%[([^%s]+) |r|cFF8800FF%- (.*)%]|h");
			if (characterName and displayName) then
				characterName = characterName:gsub("|c[Ff][Ff]......", ""):gsub("|r", "");
				displayName = displayName:gsub("|c[Ff][Ff]......", ""):gsub("|r", "");
...
function Favourites:AddItemID(itemID)
    local exists = GetItemInfo(itemID)
	if itemID and exists and not self.activeList[itemID] then
		...
		elseif type(icon) == "string" then
			if strsub(icon, 1, 1) == ATLAS_ICON_IDENTIFIER then
				if icon and icon ~= texture:GetAtlas() then
					texture:SetTexture(strsub(icon, 2))
				end
			else
				texture:SetTexture(icon)
			end
...
function Favourites:ExportItemList(listID, isGlobalList)
	...
	for entry in pairs(list) do
		if strsub(entry, 1, 2) ~= "__" and type(entry) == "number" then
	...
	if type(newList) == "string" then
		...
		for i = 1, #stList do
			...
			if entry then
                ...
                local exists, _, _, _, _, _, _, _, itemEquipLoc = GetItemInfo(entry)
				if eType == "i" and not list[entry] and exists then
					list[entry] = true
