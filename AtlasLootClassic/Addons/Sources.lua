...
local TEXTURE_ICON_F, TEXTURE_ICON_FN = "|T%s:0|t ", "|T%d:0|t "
...
local ICON_TEXTURE = {
    ...
    [1]  = format(TEXTURE_ICON_F, "Interface\\GossipFrame\\VendorGossipIcon"),   -- Loot
    [2]  = format(TEXTURE_ICON_F, "Interface\\GossipFrame\\AvailableQuestIcon"),             -- Quest
    [3]  = format(TEXTURE_ICON_F, "Interface\\GossipFrame\\BankerGossipIcon"),              -- Buy
	[4]  = format(TEXTURE_ICON_F, "interface\\icons\\spell_holy_sealofsacrifice"),   -- First Aid
	[5]  = format(TEXTURE_ICON_F, "interface\\icons\\trade_blacksmithing"),   -- Blacksmithing
	[6]  = format(TEXTURE_ICON_F, "interface\\icons\\trade_leatherworking"),   -- Leatherworking
	[7]  = format(TEXTURE_ICON_F, "interface\\icons\\trade_alchemy"),   -- Alchemy
	[8]  = format(TEXTURE_ICON_F, "interface\\icons\\trade_herbalism"),   -- Herbalism
	[9]  = format(TEXTURE_ICON_F, "interface\\icons\\trade_cooking"),   -- Cooking
	[10] = format(TEXTURE_ICON_F, "interface\\icons\\trade_mining"),   -- Mining
	[11] = format(TEXTURE_ICON_F, "interface\\icons\\trade_tailoring"),   -- Tailoring
	[12] = format(TEXTURE_ICON_F, "interface\\icons\\trade_engineering"),   -- Engineering
	[13] = format(TEXTURE_ICON_F, "interface\\icons\\trade_engraving"),   -- Enchanting
	[14] = format(TEXTURE_ICON_F, "interface\\icons\\trade_fishing"),   -- Fishing
    [15] = format(TEXTURE_ICON_F, "interface\\icons\\trade_skinning"),   -- Skinning
    [16] = format(TEXTURE_ICON_F, "interface\\icons\\trade_brewpoison"),   -- Rogue: Poisons
    [17] = format(TEXTURE_ICON_F, "interface\\icons\\inv_misc_gem_01"),   -- Jewelcrafting
    [18] = format(TEXTURE_ICON_F, "interface\\icons\\inv_inscription_tradeskill01"),     -- Inscription
}
...
local function GetTokenIcon(token)
    ...
    if token >= TOKEN_NUMBER_DUMMY then
        ...
        return format(TEXTURE_ICON_F, GetItemIcon(token))
    end
end
...
local function OnTooltipSetItem_Hook(self)
    if not SOURCE_DATA or not Sources.db.enabled then return end
    ...
    if item then
        ...
        for i = #SOURCE_DATA, 1, -1 do
            if item and SOURCE_DATA[i].ItemData[item] then
                if not sourceDataList then
                    sourceDataList = {}
                end
                sourceDataList[#sourceDataList + 1] = SOURCE_DATA[i]
            end
        end

        local newAdded
        -- sources from loot tables
        if sourceDataList  and TooltipTextCache[item] ~= false and not TooltipTextCache[item] then
            TooltipTextCache[item] = {}
            for _, sourceData in ipairs(sourceDataList) do
                ...
                if type(sourceData.ItemData[item]) == "number" then
                    ...
                    if sourceData.ItemData[item]["token"] then
                        for i, v in ipairs(sourceData.ItemData[item]["token"]) do
