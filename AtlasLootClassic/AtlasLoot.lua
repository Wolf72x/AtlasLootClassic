...
function AtlasLoot:OnInitialize()
	...
	self.dbGlobal.__addonrevision = self.__addonrevision
...
local CLASS_COLOR_FORMAT = "|cff%s%s|r"
...
function AtlasLoot:GetColoredClassNames()
	if not CLASS_NAMES_WITH_COLORS then
		...
		for class, color in pairs(RAID_CLASS_COLORS) do
			...
			local colorStr = format("%02x%02x%02x",
					color.r * 255,
					color.g * 255,
					color.b * 255
				)
			...
			if colorStr then
				CLASS_NAMES_WITH_COLORS[class] = format(CLASS_COLOR_FORMAT, colorStr, AtlasLoot.IngameLocales[class] or class)
			end
...
function AtlasLoot.SendAddonVersion(channel, target)
	...
	if channel == "PARTY" and not UnitInParty("player") then return end
	if channel == "RAID" and not UnitInRaid("player") then return end
